...
# =============================================================================
# â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ  CONFIGURATION - EDIT THIS SECTION â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ
# =============================================================================
...
# =============================================================================
# â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ  END OF CONFIGURATION - DON'T EDIT BELOW THIS LINE â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ
# =============================================================================
...
def print_score_bar(dimension: str, score: int, max_score: int = 10):
    ...
    print(f"  {dimension:22} [{color}{'â–ˆ' * bar_filled}{Colors.DIM}{'â–‘' * bar_empty}{Colors.RESET}] {color}{score:2}/{max_score}{Colors.RESET}")
...
class GroqProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        # Use openai SDK pointed at Groq — avoids urllib TLS issues on Windows
        try:
...
class JudgeSimulator:
    ...
    def run(self, scenario: str) -> bool:
        print_header(f"LLM JUDGE â€” {scenario.upper()}")
    ...
    def _warmup(self) -> bool:
        ...
        if err:
            ...
            print_success(f"metadata â€” Team: {data.get('team_name', '?')}, Model: {data.get('model', '?')}")
    ...
    def _phase2_short(self) -> bool:
        ...
        if not actions:
            print_warn("No actions â€” bot chose not to send")
            return True
    ...
    def _auto_reply(self) -> bool:
        ...
        for i in range(1, 5):
            ...
            if action == "end":
                print_success(f"Turn {i}: Bot ENDED â€” detected auto-reply pattern!")
                return True
            elif action == "wait":
...
def main():
    print_header("magicpin AI Challenge â€” LLM Judge")
